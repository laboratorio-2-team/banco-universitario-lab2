...
export const validationSchemaRegister: Schema = object({
    ...
    document_number:string().min(6).test((value)=>{if(value) return /^[0-9]*$/.test(value);}).required("Requerido"),
    birth_date:string().test((value)=>{if(value) return /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/.test(value)}).required("Requerido"),
    phone_number:string().min(2).test((value)=>{if(value) return /^\+?[0-9]*$/.test(value)}),
