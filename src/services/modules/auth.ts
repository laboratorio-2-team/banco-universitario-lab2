...
export const loginApi = async (loginInfo:LoginData) => {
    try {
        const response: AxiosResponse = await instance.post(`/v1/public/client/user/login`,loginInfo);
...
export const registerApi = async (registerInfo:RegisterData) =>{
    try {
        const response: AxiosResponse = await instance.post(`/v1/public/client/user/register`,registerInfo);
        const serviceResponse = response.data;
        return serviceResponse;
    } catch (error) {
        const errors = error as AxiosError;
